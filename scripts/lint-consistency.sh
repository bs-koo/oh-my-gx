...
echo "[8/30] Skill 체이닝 스킬의 Skill 선언"
...
echo "[9/30] gx-humanizer 에이전트 접두사"
...
echo "[10/30] force-push deny 패턴 (bare 형태 커버)"
...
echo "[12/30] gx-dev CORE 모드 계약 정합"
...
echo "[13/30] gx-tdd CORE 모드 계약 정합"
...
echo "[14/30] 모델 프로파일(standard/eco) 계약 정합"
...
echo "[15/30] 번들 경로 규약(상대경로) + config 부트스트랩"
...
echo "[16/30] phase-complete context 커밋 예외 대칭 + 레거시 Read 제거"
...
echo "[18/30] SVN .dev/.active 포인터 계약"
for f in .claude/skills/gx-dev/phases/phase-setup.md .claude/skills/gx-tdd/phases/phase-setup.md; do
...
echo "[19/30] cross-review fallback 3원 조건 + humanizer P1 커버리지"
...
echo "[20/30] 언어 중립화(projectTypes SSOT) 계약 정합"
...
echo "[22/30] 리뷰 후속(v1.21.1) 계약 정합"
...
# 소비 프로젝트의 cwd에는 scripts/가 없어 plan-lint를 실행할 수 없고, 번들 경로 규약([15/30])이
...
echo "[26/30] implement report 계약"
...
echo "[27/30] security_verdict fail-closed 계약"
