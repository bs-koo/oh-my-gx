...
# 32. gx-tdd 지시문 바이트 예산 (SKILL.md ≤ 63000B · phase-setup.md ≤ 22000B)
# 33. gx-tdd 세션 IMPLEMENT 계약 (세션 절차·--isolated 3곳·무결성 검증·격상 디스패치·태스크 수 가드·테스트 집합)
...
echo "[8/35] Skill 체이닝 스킬의 Skill 선언"
...
echo "[9/35] gx-humanizer 에이전트 접두사"
...
echo "[10/35] force-push deny 패턴 (bare 형태 커버)"
...
echo "[12/35] gx-dev CORE 모드 계약 정합"
...
echo "[13/35] gx-tdd CORE 모드 계약 정합"
...
echo "[14/35] 모델 프로파일(standard/eco) 계약 정합"
...
echo "[15/35] 번들 경로 규약(상대경로) + config 부트스트랩"
...
echo "[16/35] phase-complete context 커밋 예외 대칭 + 레거시 Read 제거"
...
echo "[18/35] SVN .dev/.active 포인터 계약"
for f in .claude/skills/gx-dev/phases/phase-setup.md .claude/skills/gx-tdd/phases/phase-setup.md; do
...
echo "[19/35] cross-review fallback 3원 조건 + humanizer P1 커버리지"
...
echo "[20/35] 언어 중립화(projectTypes SSOT) 계약 정합"
...
echo "[22/35] 리뷰 후속(v1.21.1) 계약 정합"
...
# 소비 프로젝트의 cwd에는 scripts/가 없어 plan-lint를 실행할 수 없고, 번들 경로 규약([15/35])이
...
echo "[26/35] implement report 계약"
...
echo "[27/35] security_verdict fail-closed 계약"
...
[ "$FAIL" -eq 0 ] && ok "린트 번호 크로스레퍼런스 ${LINT_TOTAL}건 정합 확인"

echo "[32/35] gx-tdd 지시문 바이트 예산"
# 오케스트레이터가 사이클마다 읽는 파일의 상한. 설계: docs/specs/2026-09-07-tdd-density-rhythm-design.md D3
for spec in ".claude/skills/gx-tdd/SKILL.md:63000" ".claude/skills/gx-tdd/phases/phase-setup.md:22000"; do
  f=${spec%%:*}; max=${spec##*:}
...
[ "$FAIL" -eq 0 ] && ok "SKILL.md ≤ 63000B · phase-setup.md ≤ 22000B · 조건부 로드 포인터 4건"

echo "[33/35] gx-tdd 세션 IMPLEMENT 계약"
...
echo "[35/35] gx-tdd 구현 내부 게이트 판정 계약"
