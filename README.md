...
- 자동화는 PR 생성까지만입니다. **PR 머지는 사용자가 직접** 합니다. `git push --force`, `gh pr merge`는 설정 수준에서 막혀 있습니다.
...
아닙니다. "이어서 해줘"라고 말하면 `.dev/state.md`에 저장된 진행 상태를 읽어 멈춘 단계부터 다시 시작합니다. `dev`와 `tdd`는 서로의 진행 상태를 구분하므로 잘못된 파이프라인으로 재개되는 일은 없습니다.
</details>
