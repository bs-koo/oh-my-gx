...
"TDD로", "테스트 먼저" 같은 명시적 키워드가 있으면 `tdd`로, 없으면 `dev`로 갈립니다.
...
내부에서는 에이전트 팀이 요구사항 → 설계 → 구현 → 리뷰 → 완료로 단계를 나눠 처리합니다.

### tdd

`dev`와 같은 6단계 골격에 **TDD 게이트를 강제로 끼운** 파이프라인입니다. 구현은 RED-GREEN-REFACTOR 사이클로 격리되고, 완료 전에는 verify 게이트를 반드시 통과해야 합니다.
...
`/oh-my-gx:setup`에서 Google Chat 웹훅을 연동해두면 PR이 생성될 때 Chat Space로 알림이 자동 전송됩니다. 한 명이 설정해 커밋하면 팀 전체가 알림을 받습니다.
...
일반 기능 개발이라면 `dev`, 테스트를 먼저 짜고 회귀를 막아야 하는 작업이라면 `tdd`입니다. "TDD로", "테스트 먼저" 같은 키워드를 쓰면 `tdd`가 자동으로 발동합니다. 애매할 때는 어느 방식으로 갈지 물어봅니다.
</details>
...
네. `/gx-setup`을 실행하면 VCS를 자동으로 감지합니다. SVN 프로젝트에서도 PRD·설계·구현·리뷰까지 똑같이 동작하고, 커밋만 `svn commit`으로 직접 하면 됩니다.
</details>
