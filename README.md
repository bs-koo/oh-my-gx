...
계획에 그 ID의 행이 있으면 도메인·요구사항·브랜치명을 알아서 확정합니다. 무엇을 할 수 있는지 모를 때는 이렇게 물으면 됩니다.
...
`.dev/plan.md`는 저장소에 공유됩니다. 착수하면 그 사실이 베이스 브랜치로 바로 반영되고 완료 시 상태가 갱신되므로, 팀원이 `git pull` 후 누가 무엇을 하고 있는지 봅니다. 선행 작업이 끝나지 않았으면 경고합니다.
