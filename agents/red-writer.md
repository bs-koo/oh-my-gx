...
모의(mock)를 사용할 때 `.claude/skills/gx-tdd/references/testing-anti-patterns.md`의 게이트를 적용한다:

- 모의의 동작이 아니라 **실제 동작**을 검증한다.
- 모의 구조는 설계서 testability 섹션의 인터페이스만 근거로 구성한다. 없는 필드를 추측하지 않으며, 부족하면 **"설계서 인터페이스 불충분"**으로 보고한다.
