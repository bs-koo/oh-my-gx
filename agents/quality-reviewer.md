...
- 컨벤션 위반 (네이밍, 들여쓰기 등)
- 부적절한 에러 핸들링
- 테스트 코드 품질 (모의 동작 검증, 프로덕션 클래스의 테스트 전용 메서드, 불완전 모킹 — 상세 기준: 디스패치 프롬프트로 전달받은 testing-anti-patterns.md 경로, 미전달·부재 시 괄호 안 항목 정의로 판정. 프로덕션 동작 불변이므로 `[동작불변]` 표기)
...
- **동작 결함** (Critical 전부 + Important 중 동작 변경을 동반하는 항목 — 잘못된 에러 핸들링, 누락된 분기, race condition 등): 새 AC로 정의하여 **RGR 사이클**(red-writer → implementer)로 수정. 결함을 재현하는 실패 테스트가 선행되어야 한다.
