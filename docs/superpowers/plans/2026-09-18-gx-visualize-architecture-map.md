...
### Task 13~15: PR 이후 사용자 피드백 반영

PR #92가 올라간 뒤 **사용자가 실제 생성된 산출물을 열어보고** 나온 작업이다. 전부 실물을 확인해야만 보이는 문제였고, 단위 테스트는 모두 통과하고 있었다.

**Task 13 — 산출물 정리와 폴백 그림** (`6e6f295`·`b719256`·`c303873` + `479021b`)

지적 1: *"파일이 너무 많아서 뭐가 뭔지도 잘 모르겠어. 정리를 폴더로 해서 딱 한 파일만 html 이름을 지정해서 보여주도록"* — 8도메인 × 4파일 = 32개가 평평하게 쌓여 있었다. `${MAP_DIR}`를 `아키텍처-맵.html`(인덱스) + `domains/`·`ir/`·`receipts/`·`assets/`로 정리했다. 인덱스는 파일 목록이 아니라 판단 정보를 담는다 — 도메인별 노드 수, 그림인가 표인가, 누락 건수(0이면 "누락 없음"으로 명시).

지적 2: *"가장 중요한 다이어그램은 그려서 보여지지 않고 있는데"* — 폴백 도메인이 Mermaid 소스만 `<pre>`로 뿌리고 렌더하지 않았다. `mermaid.min.js`(실측 3.4MB)를 `assets/`에 **한 벌만** 두고 폴백 HTML들이 참조하게 했다 — 파일별이 아닌 **폴더 단위 self-contained**이며 오프라인에서도 동작한다.

함께 고친 결함 3건(컨트롤러가 `reb.html`을 열어 발견): Mermaid 소스가 모든 문자를 `#NNN;`로 인코딩해 읽을 수 없던 것, 라벨이 긴 노드 ID로 시작하던 것, `<h3>`에 `overflow-wrap`이 없어 카드 경계를 침범하던 것.

`479021b`은 컨트롤러가 산출물을 재생성하다 찾은 것이다 — `SKILL.md`의 렌더 예시 명령 **두 곳에 `--project-root`가 빠져** 있어 문서대로 따르면 검증 실패로 그림이 만들어지지 않았다. "IR을 넘기는 모든 렌더 예시가 그 인자를 갖는지" 검사하는 계약 테스트를 함께 넣었고, **그 테스트가 두 번째 누락을 찾아냈다.**

**Task 14 — 끊긴 체인과 라벨 잡음** (`d08827e`·`848578a`·`d5b5d46`)

지적 3: *"다 확인 필요라고 나오고 이게 옳게 된 다이어그램 맞아? 너무 간단한데"* — 지적이 정확했다. **인터페이스+구현체 쌍 9개 전부**가 엣지로 이어지지 않아 다이어그램이 두 덩어리로 갈라져 있었다. 컨트롤러는 인터페이스(`UserService`)를 주입받고 실제 구현은 `UserServiceImpl`인데 둘 사이 엣지가 없었다. eGov·Spring 표준 구조라 **모든 GX 프로젝트에서 발생**한다.

사용자 결정에 따라 **하나로 합쳤다** — 살아남는 노드는 인터페이스이고, 구현체의 엣지와 `evidence`를 그쪽으로 옮긴다. 근거는 잃지 않는다(`UserService` 노드가 `UserService.java:18`·`UserServiceImpl.java:45` 둘 다 보유). 실측: 노드 86→77(정확히 9 감소), **엣지는 86으로 동일**.

라벨 잡음 2건도 고쳤다 — 모든 노드가 `status: unknown`이라 "확인 필요"가 박스마다 찍히던 것, 테이블의 `label`과 `technical_label`이 같아 `TB_ROLE TB_ROLE`로 두 번 찍히던 것.

`d5b5d46`은 리뷰가 잡은 범위 문제다. `render_fallback.py`는 5개 view 공용이라 `unknown` 억제가 전역이면 **사람이 채우는 `trace`·`progress`·`impact`에서 "아직 확인 안 됨"이라는 실행 가능한 신호가 사라진다.** `service` 뷰로 좁히고 양방향을 구속하는 테스트를 넣었다.

**Task 15 — 셸 인자 손상 제거**

Task 13 리뷰어가 `SKILL.md` 예시를 **셸에서 문자 그대로** 실행해 찾았다. `--archify-command`에 JSON 배열 문자열을 셸로 넘기면 git-bash는 백슬래시가 한 겹 손실돼 `shlex.split` 폴백이 `Program Files`의 공백에서 잘못 쪼개고, PowerShell은 큰따옴표가 사라져 다르게 깨진다 — **두 셸 모두 실패**하며 해당 도메인이 mermaid로 조용히 폴백된다.

이 스킬의 실제 실행자는 Bash 도구를 쓰는 에이전트이므로 실제 경로가 셸을 거친다. 지금까지 종단 확인이 성공한 것은 Python 안에서 셸을 거치지 않았기 때문이었다.

이스케이프를 정교하게 만드는 대신 **직렬화 표면 자체를 없앤다** — `--archify-command`를 선택 인자로 바꾸고 생략하면 `ensure_archify()`로 스스로 찾는다.
...
| 18. 산출물이 폴더로 정리되고 인덱스 하나로 파악된다 | Task 13 |
| 19. 폴백 도메인도 브라우저에서 다이어그램이 그려진다 | Task 13 |
| 20. 인터페이스·구현체가 합쳐져 체인이 끊기지 않는다 | Task 14 (`test_merged_service_keeps_the_impl_outgoing_edges`) |
| 21. 문서화된 명령이 셸을 거쳐도 동작한다 | Task 15 |
