...
- 설계 방향 (상세는 v1.17.0 별도 spec):
  ...
  - 구현은 Task 디스패치의 model 파라미터 오버라이드(frontmatter보다 우선)로 — `agents/*.md` 무수정. ~~하향 대상은 dev·tdd의 opus 5종~~ → 구현 시 개정: **architect는 유지**하고 dev 2종(design-critic·coder)·tdd 3종(design-critic·test-architect·quality-reviewer)만 하향한다 (eco spec §4 "게이트 무방비 산출물의 생산자만 opus 유지" 원칙).
...
## 12. v1.18.0 개정: 레거시 모드 호환 전면 제거

- 배경: `--hotfix` 플래그·구 세션 마이그레이션(`normal`/`full`/`light`/`hotfix`/`implement`→`all`/`core`)·구 Trust Ledger 섹션명(`### Light/Hotfix 긴급 감사`) 인식 등 레거시 호환이 v1.16~v1.17에 누적되어 문서·린트·골든의 유지 비용이 커졌다. `full`/`light`/`normal`은 릴리스된 적 없는 v1.16 개발 중 중간 명칭, `hotfix`/`implement`는 v1.15 이하 레거시이며, `--hotfix`는 자연어 "긴급/핫픽스"(→ 핵심 모드)로 완전 대체 가능했다.
- 결정(breaking): 레거시 모드 호환을 **전면 제거**한다. `--hotfix` 플래그 지원 중단(자연어 "긴급/핫픽스"는 핵심 모드 라우팅으로 유지), 구 세션 재개 마이그레이션을 **구 버전 세션 방어**(`mode`가 `all`/`core`가 아니면 재개 거부·안내)로 대체, 구 산출물 섹션명 인식 제거. §10~§11.4의 레거시 유지 서술(예: §11.4의 `--hotfix` 매핑 검사, S13 마이그레이션 확장)은 본 절로 개정된다.
- 함께 반영(정합성 갭): 린트 [12]/[13]의 레거시 매핑 존재 검사를 '구 버전 방어 존재 + `hotfix`/`light` 잔존 금지'로 반전, [14] opus↔eco 대조를 하이픈 경계 정규식으로 교정(`refactor-coder`의 `coder` 부분 문자열 오탐 제거), 골든 S13 반전·S17(gx-tdd eco 디스패치) 신설(16→17), gx-ralph의 `model-profile` 무인 루프 무시·복귀 후 재적용 명시.
