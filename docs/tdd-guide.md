...
| 최소 구현 | "이왕 하는 김에" 과잉 구현 | IMPLEMENT YAGNI(세션·implementer 공통) + 과잉 구현 감지 |
...
TDD의 각 단계를 **다른 주체가 맡는다** (IMPLEMENT는 기본 세션). 한 에이전트가 테스트와 구현을 모두 쓰면 구현에 편향된 테스트가 나오기 때문이다.

| 단계 | 주체 | 무엇을 보는가 | 무엇이 금지되는가 |
|------|---------|-------------|-----------------|
...
| 리뷰 | `reviewer` | PRD의 AC + diff + 컨벤션 (Part 1: AC 충족 → Part 2: 코드 품질). 태스크 범위 모드(Step 2-V)는 태스크 AC + 태스크 diff만 | Part 1 verdict 확정 전 품질 판정 |
...
4. 테스트에서 쓰이지 않는 public 메서드·필드가 추가됐으면 과잉 구현으로 판정해 제거한다 (설계서 인터페이스가 명시한 멤버는 유지). 묻지 않고 처리하고 판정·근거·되돌림 비용을 `decisions.md`에 남긴다
