...
| LIGHT | setup → light(AC 확인 → 구현 → Gate → 기록) → complete | 소형 변경 — 기록·게이트 유지, 에이전트 디스패치 0~1회 |
| LIGHT (긴급 프리셋) | 동일 (AC 확인 질문 생략, AC를 재현 조건 형식으로) | 긴급 버그 수정 (레거시 `--hotfix` 매핑) |
...
|------|------|------|
| normal | setup → requirements → design → implement → review → complete | 전체 파이프라인 |
| light | setup → light(AC 확인 → 구현 → Gate → 기록) → complete | 소형 변경. 기록(ac.md·summary.md)과 빌드·테스트 게이트는 유지 |
| light (긴급 프리셋) | 동일 — AC 확인 질문 생략, AC를 재현 조건 형식으로 | 긴급 버그 수정 (레거시 `--hotfix`가 여기로 매핑) |
...
| 미지원 | gx-dev의 라이트(light) 경량 모드 (Iron Law 충돌로 의도적 제거 — 긴급 건은 gx-tdd 자체 hotfix 모드 사용, 테스트 강제 없는 경량 경로는 gx-dev 라이트 모드 사용) |
