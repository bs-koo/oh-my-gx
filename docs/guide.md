...
| implement | coder + qa-manager | 구현 + 자기점검. 진입 시 "대화형 구현 vs ralph 무인 루프"를 1회 확인 — 전환 시 §4.13으로 이어짐 (hotfix·경량 구현·svn·`--phase`/`--resume`은 질문 생략) |
...
| 미지원 | gx-dev의 "구현만" 경량 모드 (Iron Law 충돌로 의도적 제거 — 긴급 건은 hotfix 모드 사용) |
