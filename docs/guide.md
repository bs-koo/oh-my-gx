...
/gx-context ──────── 도메인 지식 등록/갱신
     ...
     │               /gx-tdd ──── PRD → 설계 → RED-GREEN-REFACTOR → 리뷰 → verify → 커밋 → PR
     │                     │        (단독 보조: /gx-red /gx-green /gx-refactor /gx-verify)
     │                     ↓
     │               /gx-pull-request ── PR 생성
     │               /gx-cross-review ── dev/tdd 산출물 교차 검증 (완료 후 단발)
     │               /gx-ralph ──────── PRD 확정 후 AC 무인 반복 루프 준비
     │                                   (외부 러너 scripts/gx-ralph.sh, 내부: /gx-ralph-iterate)
     │
/gx-lens ─────────── 현행 정책 분석 + 영향도 (읽기 전용)
/gx-tech-debt ────── 기술 부채 분석 + 우선순위 로드맵 (읽기 전용)
/gx-research ─────── 외부 리서치 → context 반영 가능
/gx-humanizer ────── AI 글쓰기 교정 (audit / rewrite / strict)
```
...
`/gx-dev`를 실행하면 **오케스트레이터**가 기동합니다. 오케스트레이터는 `SKILL.md`(`.claude/skills/gx-dev/SKILL.md`)에 정의된 프로토콜에 따라 Phase를 순차적으로 실행하는 지휘자입니다.

#### 오케스트레이터의 역할

1. **Phase 파일 로드**: 각 Phase에 진입할 때 해당 Phase 파일(`.claude/skills/gx-dev/phases/phase-{name}.md`)을 Read하여 지시사항을 메모리에 로드합니다.
...
| 모드 | Phase 경로 | 용도 |
|------|-----------|------|
| 전체(all) | setup → requirements → design → implement → review → complete | 전체 과정 진행 |
| 핵심(core) | setup → core(AC 확인 → 구현 → Gate → 기록) → complete | 소형 변경 — 기록·게이트 유지, 에이전트 디스패치 0~1회. 긴급 버그 수정도 이 모드 (AC를 재현 조건 형식으로, 레거시 `--hotfix` 매핑) |

#### 모델 프로파일 (표준/에코)

절차 축(모드)과 직교하는 두 번째 축입니다. 절차·게이트는 동일하고 **에이전트 디스패치 모델**만 달라집니다:

| 프로파일 | 동작 | 용도 |
|---------|------|------|
| 표준(standard) | Agent 팀 표의 모델 그대로 (설계·구현·품질 리뷰 = opus) | 품질 우선 — Max 요금제 권장 |
| 에코(eco) | architect를 제외한 opus 에이전트를 sonnet으로 하향 디스패치 (Task model 파라미터 오버라이드 — 설계는 opus 유지) | 토큰 절약 — Pro 요금제 권장. 빌드·테스트·verify 게이트는 동일 |

결정 우선순위: `--eco`/`--standard` 플래그 > 자연어("에코 모드", "에코로", "절약 모드") > 모드 확인 질문의 프로파일 답변 > config.json `modelProfile`(`/gx-setup`에서 1회 설정) > 기본 표준. 일반 기능 요청처럼 모드 확인 질문이 뜨는 경우에는 **표준/에코 질문이 같은 창에 함께 제시**되어 한 번의 submit으로 두 축을 결정합니다 (dev·tdd 동일). 모드가 자동 판정되면(긴급·구현만 등) 프로파일 질문 없이 config/기본값으로 결정됩니다.
...
for phase in PHASES:
    ...
    if phase == "design" and not exists(".dev/{branch-slug}/prd.md"):
        → phase-requirements부터 실행
    if phase == "implement" and not exists(".dev/{branch-slug}/design.md"):
        → phase-design부터 실행  (핵심 모드는 implement 대신 core phase를 사용하므로 이 게이트 미적용)

    # 2. Phase 파일 Read
    Read(".claude/skills/gx-dev/phases/phase-{phase}.md")
...
| 모드 | 경로 | 용도 |
|------|------|------|
| all | setup → requirements → design → implement → review → complete | 전체 과정 진행 |
| core | setup → core(AC 확인 → 구현 → Gate → 기록) → complete | 소형 변경. 기록(ac.md·summary.md)과 빌드·테스트 게이트는 유지. 긴급 버그 수정도 이 모드 — AC를 재현 조건 형식으로 (레거시 `--hotfix` 매핑) |
...
| 자연어 표현 | 내부 동작 |
|-------------|-----------|
| "긴급", "핫픽스", "빨리 고쳐" | 핵심 모드 (AC를 재현 조건 관점으로) |
| "구현만", "핵심만", "라이트", "가볍게" | 핵심 모드 |
| "에코 모드", "에코로", "절약 모드" | 에코 프로파일 — 에이전트를 sonnet 중심으로 하향 (모드와 독립. 단독 명사 "에코"는 오탐 방지를 위해 매칭 제외) |
| "이어서", "계속", "재개" | 이전 작업 재개 |
| "어디까지", "상태", "현황" | 진행 상태 조회 |
...
| ac.md | 핵심 모드 AC (초경량 PRD: 배경 + 요구사항) |
| summary.md | 핵심 모드 변경 요약 + Gate 증거 |
...
- **strict** -- rewrite 후 humanizer-fidelity(의미 보존)·humanizer-naturalness(과윤문/잔존) 에이전트가 교차 검증. 8,000자 초과 시 자동 승급
...
단독·체이닝 전용 스킬입니다 (gx-tdd 파이프라인은 이 스킬들을 거치지 않고 에이전트를 직접 디스패치합니다).

| 스킬 | 역할 |
|------|------|
| `/gx-red` | 실패 테스트 우선 작성 (프로덕션 코드 금지) |
| `/gx-green` | 실패 테스트를 통과시키는 최소 구현 (green-coder 재호출 최대 2회) |
| `/gx-refactor` | GREEN 상태를 유지하며 중복 제거·정리 (동작 변경 금지, 깨지면 롤백) |
| `/gx-verify` | 완료 검증 게이트 — 테스트 직접 실행, 0 failures·경고 baseline 비교 |

### 4.11 `/gx-tech-debt` -- 기술 부채 분석

코드베이스의 기술 부채를 탐지하고 우선순위 로드맵을 제공합니다. 읽기 전용입니다.

### 4.12 `/gx-cross-review` -- 산출물 교차 검증

`/gx-dev`·`/gx-tdd` 완료 후 단발로 호출합니다. PRD/설계서/Trust Ledger 등 dev 산출물을 컨텍스트로 주입해 "산출물 약속 대비 충실도"를 검증합니다 (일반 코드 리뷰와 관점이 다릅니다).
...
| `--core/--hotfix와 --phase는 동시에 사용할 수 없습니다` | 플래그 충돌 | 하나만 사용 |
