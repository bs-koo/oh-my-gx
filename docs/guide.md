...
| 핵심(core) | setup → core(AC 확인 → 구현 → Gate → 기록) → complete | 소형 변경 — 기록·게이트 유지, 에이전트 디스패치 0~1회. 긴급 버그 수정도 이 모드 (AC를 재현 조건 형식으로) |
...
| core | setup → core(AC 확인 → 구현 → Gate → 기록) → complete | 소형 변경. 기록(ac.md·summary.md)과 빌드·테스트 게이트는 유지. 긴급 버그 수정도 이 모드 — AC를 재현 조건 형식으로 |
