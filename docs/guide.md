...
| 모드 | Phase 경로 | 용도 |
|------|-----------|------|
| FULL | setup → requirements → design → implement → review → complete | 전체 파이프라인 |
| LIGHT | setup → light(AC 확인 → 구현 → Gate → 기록) → complete | 소형 변경 — 기록·게이트 유지, 에이전트 디스패치 0~1회. 긴급 버그 수정도 이 모드 (AC를 재현 조건 형식으로, 레거시 `--hotfix` 매핑) |
...
for phase in PHASES:
    ...
    if phase == "implement" and not exists(".dev/{branch-slug}/design.md"):
        → phase-design부터 실행  (LIGHT 모드는 implement 대신 light phase를 사용하므로 이 게이트 미적용)
...
|------|------|------|
| full | setup → requirements → design → implement → review → complete | 전체 파이프라인 |
| light | setup → light(AC 확인 → 구현 → Gate → 기록) → complete | 소형 변경. 기록(ac.md·summary.md)과 빌드·테스트 게이트는 유지. 긴급 버그 수정도 이 모드 — AC를 재현 조건 형식으로 (레거시 `--hotfix` 매핑) |
...
| light | (오케스트레이터, 규모에 따라 coder) | AC 작성·확인 → 구현 → Mechanical Gate(빌드+테스트 필수) → 기록. LIGHT 모드 전용 |
| review | qa-manager + security-auditor | 코드 리뷰 + 보안 감사 (병렬) |
| complete | product-owner | 인수 검증 → 커밋 → PR. LIGHT 모드는 product-owner 대신 AC 자가 검증 |

#### 인자/옵션 (자연어로 사용 가능)

| 자연어 표현 | 내부 동작 |
|-------------|-----------|
| "긴급", "핫픽스", "빨리 고쳐" | light 모드 (AC를 재현 조건 관점으로) |
| "구현만", "라이트", "가볍게" | light 모드 |
| "이어서", "계속", "재개" | 이전 작업 재개 |
| "어디까지", "상태", "현황" | 진행 상태 조회 |
...
| ac.md | LIGHT 모드 AC (초경량 PRD: 배경 + 요구사항) |
| summary.md | LIGHT 모드 변경 요약 + Gate 증거 |
...
| light 모드 | gx-dev light와 달리 **RGR·verify·G-W-T 게이트·긴급 보안 감사를 전부 유지**하는 경량 경로 (Iron Law 불변). 생략은 design(testability)·정식 review뿐이며, AC 작성이 product-owner 대신 오케스트레이터 직접(ac.md). 테스트 강제 없는 경량 경로가 필요하면 gx-dev light 사용 |
...
| `--light/--hotfix와 --phase는 동시에 사용할 수 없습니다` | 플래그 충돌 | 하나만 사용 |
