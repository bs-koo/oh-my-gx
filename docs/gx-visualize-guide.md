...
| `all` | `docs/architecture/`(`--map-dir`로 변경 가능) | 항상 최신으로 증분 갱신, 도메인별로 분할 |

두 산출물은 한 폴더에 섞이지 않습니다. `--scope session` HTML에는 생성 시각과 커밋 해시가 담긴 스냅샷 배너가 붙고, `--scope all`에는 붙지 않습니다.

### `--scope all`의 산출물은 도메인별입니다

실제 저장소 규모를 한 장으로 그리면 Archify 검증이 대량으로 실패하고 사람이 읽기도 어렵습니다. 그래서 `--scope all`은 노드를 도메인별로 나눠 각각 별도 문서로 그립니다 — 파일명은 `{domain}.ir.json`·`{domain}.html`(예: `auth.ir.json`, `auth.html`)이고, 도메인 전체가 공유하는 `.scan-manifest.json`이 함께 저장됩니다. `--scope session`은 분할하지 않습니다. `--domain`을 주면 그 도메인만 그리고, 생략하면 찾은 전 도메인을 각각 그립니다.
...
`${MAP_DIR}/.scan-manifest.json`이 없으면 이번 실행이 **최초 전체 스캔**이라는 뜻이고 프로젝트 전체를 읽으므로 시간이 걸립니다 — 실행 전에 사용자에게 먼저 알립니다. 두 번째 실행부터는 매니페스트에 기록된 파일 지문과 현재 파일을 비교해 바뀌거나 삭제된 파일만 다시 스캔합니다.

지문은 **Git 프로젝트에서는 `git hash-object`(파일 내용 해시)**를 사용합니다. **Git이 아니거나 `git hash-object` 실행이 실패하면 `mtime_ns-size`(수정 시각 나노초와 파일 크기)로 대체**합니다 — 내용이 같아도 파일을 touch만 하면 변경으로 잡힐 수 있다는 뜻이므로, 비-git 프로젝트에서는 실제 변경 범위보다 넓게 재스캔될 수 있습니다.
