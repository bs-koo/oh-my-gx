...
| S18 | `projectTypes` 등록된 git 프로젝트 | `/gx-tdd 랄프로 알림 임계값 검증 개발해줘` | 모드 질문 생략(all 확정) — 프로파일 질문만 제시(미확정 시) → requirements·design 정상 → Step 0.5 기준선 게이트 통과 후 질문 없이 `Skill("oh-my-gx:gx-ralph")` 호출·파이프라인 종료. state.md `flags`에 `--ralph` 기록. "랄프로 이어서 해줘"는 RESUME이 우선하고 RALPH 무시 안내 | 의도 파싱 RALPH 우선순위 규칙 + Step 3 모드 질문 생략 규칙 → phase-setup Step 7 flags → phase-implement Step 0.7 |
