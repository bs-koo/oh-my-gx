...
PHASE 매핑: `PRD만`/`요구사항만` → `--phase requirements`, `설계만` → `--phase design`, `리뷰만` → `--phase review`, `커밋만`/`PR만` → `--phase complete`. implement phase 단독 실행은 자연어 매핑 없이 `--phase implement` 플래그 전용이다 (자연어 "구현만"은 LIGHT로 라우팅 — 설계서 기반 구현 단독 실행과 의도가 다르다).
...
AskUserQuestion(
  questions: [{
    ...
    options: [
      ...
      { label: "라이트", description: "AC 확인 → 구현 → 빌드·테스트 게이트 → 기록 → PR. 소형 변경용 — 산출물(ac.md·summary.md)은 남긴다" }
    ],
    multiSelect: false
  }]
)
...
> Phase를 건너뛸 수 있는 조건은 LIGHT 모드(사용자가 명시적으로 선택한 경량 경로 — 기록·Gate는 유지), `--phase` 플래그, 그리고 phase-implement의 "구현 방식 확인"에서 사용자가 **ralph 무인 루프**를 선택한 경우(gx-ralph 호출 후 종료 — 이후 Phase는 루프 종료 후 `--phase review`/`--phase complete`로 재개)뿐이다.
...
for phase in PHASES:
    ...
    # 참고: light 모드는 PRD/설계서 대신 ac.md로 진행하므로 이 게이트를 적용하지 않는다
...
`phase-setup.md`, `phase-requirements.md`, `phase-design.md`, `phase-implement.md`, `phase-light.md`, `phase-review.md`, `phase-complete.md`
...
- LIGHT 모드: AC를 `${DEV_DIR}/ac.md`에, 변경 요약을 `${DEV_DIR}/summary.md`에 저장한다 (phase-light Step 0 / Step 3).
...
- **coder (구현)**: 설계서 전체 + 코드 맵 + 프로젝트 루트 경로 + REFERENCES (있으면). LIGHT 모드이면 설계서 대신 ac.md 전체 + 코드 맵 (phase-light Step 1의 규모 판정에서 coder 디스패치가 결정된 경우).
...
```
사용자가 "수정 요청"이나 Other를 선택하면 수정 내용을 직접 입력받는다.
