...
> Phase를 건너뛸 수 있는 조건은 `--hotfix` 모드, `implement` (경량 구현) 모드, `--phase` 플래그, 그리고 phase-implement의 "구현 방식 확인"에서 사용자가 **ralph 무인 루프**를 선택한 경우(gx-ralph 호출 후 종료 — 이후 Phase는 루프 종료 후 `--phase review`/`--phase complete`로 재개)뿐이다.
...
for phase in PHASES:
    ...
    if phase == "implement" and not implement_mode and not exists("${DEV_DIR}/prd.md"):
        → phase-requirements부터 실행
