...
- **순차 질문 루프**를 실행한다 (큐가 비워질 때까지):
  1. 큐에서 질문 1개를 꺼내 `AskUserQuestion(questions: [질문 1개])`로 제시한다. **모든 질문에 권장 답변(현재 맥락에서 가장 가능성 높은 답 또는 합리적 추정)을 options 첫 번째 항목에 `(Recommended)` 라벨로 반드시 제시한다.** 권장 답변 산출이 어려우면 product-owner에 재요청한다. AskUserQuestion 포맷은 **SKILL.md 공유 규칙**의 `에이전트 질문 → AskUserQuestion 변환 규칙`을 따른다.
  2. 답변을 평가한다:
