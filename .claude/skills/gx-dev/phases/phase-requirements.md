...
- **순차 질문 루프**를 실행한다 (큐가 비워질 때까지):
  ...
  3. 큐가 비워지면 **공유 이해 확인(align)** 단계를 거친다. 수렴된 답변을 "Q → A" 형식으로 요약하여 다음과 같이 확인한다:
     ```
     AskUserQuestion(
       questions: [{
         ...
         options: [
           ...
           { label: "수정 필요", description: "Other로 이동해서 수정/보완할 내용을 입력해주세요" }
         ],
         multiSelect: false
       }]
     )
     ```
     "맞습니다"면 product-owner에게 전달 → Step 2로 돌아간다. "수정 필요"면 해당 답변을 재정리하여 큐에 재삽입한 뒤 루프 재진입한다.
