...
- **순차 질문 루프**를 실행한다 (큐가 비워질 때까지):
  1. 큐에서 질문 1개를 꺼내 `AskUserQuestion(questions: [질문 1개])`로 제시한다. **모든 질문에 권장 답변(기존 코드 패턴·트레이드오프 분석 기반의 가장 합리적 선택)을 options 첫 번째 항목에 `(Recommended)` 라벨로 반드시 제시한다.** 권장 답변 산출이 어려우면 architect에 재요청한다. AskUserQuestion 포맷은 **SKILL.md 공유 규칙**의 `에이전트 질문 → AskUserQuestion 변환 규칙`을 따른다.
  2. 답변을 평가한다:
