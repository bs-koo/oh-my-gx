     ...
     AskUserQuestion(
       questions: [{
         ...
         options: [
           ...
           { label: "수정 필요", description: "Other로 이동해서 수정할 질문 번호와 새 답변을 입력해주세요 (예: 'Q2: 캐시 대신 매번 조회')" }
         ],
         multiSelect: false
       }]
     )
     ```
     "맞습니다"면 architect에게 전달 → 다음 반복으로 진행한다. "수정 필요"면 사용자가 Other에 입력한 `Q번호: 새 답변` 패턴을 파싱하여 해당 질문을 큐에 재삽입한 뒤 루프 재진입한다. 입력이 패턴과 일치하지 않거나 번호를 식별할 수 없으면, 어떤 항목을 수정할지 선택형 질문으로 다시 묻는다.
