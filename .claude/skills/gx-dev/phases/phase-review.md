...
- **`--phase review` 단독 실행**: 베이스 브랜치 감지 규칙에 따라 베이스를 결정한다. **Diff 수집 규칙**에 따라 `${GIT_PREFIX} diff $(${GIT_PREFIX} merge-base HEAD <base-branch>)...HEAD -- . ':(exclude).dev'`를 `DIFF_FILE`에 리다이렉트한다. 미커밋 변경이 있으면 `${GIT_PREFIX} diff >> ${DIFF_FILE}`로 추가한다.
