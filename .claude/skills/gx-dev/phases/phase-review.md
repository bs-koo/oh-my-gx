...
3. 프로젝트 타입으로도 결정 불가:
   ```
   AskUserQuestion(
     question: "빌드 검증 명령을 감지하지 못했습니다.",
     options: [
       { value: "input", label: "직접 입력 — 빌드 명령을 직접 입력합니다" },
       { value: "skip", label: "건너뛰기 — 빌드 검증 없이 진행합니다" }
     ]
   )
   ```
   - 직접 입력 → 후속 AskUserQuestion(자유입력)으로 명령을 받아 사용.
   - 건너뛰기 → 다음 단계로 진행.
...
6. **재시도 실패** → 사용자에게 빌드 에러를 표시하고:
   ...
   AskUserQuestion(
     ...
     options: [
       { value: "continue", label: "계속 진행 — 직접 수정 후 이어서 진행합니다" },
       { value: "abort", label: "중단 — 파이프라인을 중단합니다" }
     ]
   )
   ```
...
if did_fix:
    ...
    if Warning/Info(QA) 또는 HIGH/MEDIUM(ZT) 항목이 있으면:
        ...
        AskUserQuestion(
          ...
          options: [
            { value: "fix", label: "수정 — 위 항목들을 자동 수정합니다" },
            { value: "skip", label: "건너뛰기 — 현재 상태로 진행합니다" }
          ]
        )
        ```
        if 수정 선택:
...
AskUserQuestion(
  ...
  options: [
    { value: "manual", label: "수동 수정 후 재리뷰 — 직접 수정 후 다시 리뷰합니다" },
    { value: "proceed", label: "현재 상태로 진행 — 미해결 항목을 남기고 진행합니다" }
  ]
)
```
