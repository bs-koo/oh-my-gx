...
**LIGHT 모드이면** product-owner를 디스패치하지 않고 오케스트레이터가 **AC 자가 검증**을 직접 수행한다:
...
1. **args 구성**:
   - `--background ${DEV_DIR}/prd.md`: PRD의 "배경"과 "요구사항"을 Background 섹션에 반영. **LIGHT 모드이면 `--background ${DEV_DIR}/ac.md`** (ac.md도 "배경"+"요구사항" 섹션 구조라 동일하게 파싱된다. 레거시 재개로 ac.md가 없고 prd.md만 있으면 prd.md를 사용한다).
   - `--extra-section ${DEV_DIR}/trust-ledger.md`: Trust Ledger가 존재하면 Audit Summary 섹션을 Checklist 앞에 삽입. **LIGHT 모드이면 `--extra-section ${DEV_DIR}/summary.md`** (변경 요약 + Gate 증거). 해당 파일이 없으면 `--extra-section`을 생략한다.
