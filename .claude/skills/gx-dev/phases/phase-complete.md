...
3. `상태` 열을 `완료`로 바꾼다.
4. **전용 커밋**: `.dev/plan.md`만 스테이징하여 `docs: [plan] {ID} 완료` 메시지로 커밋하고 push한다. 기능 코드가 아닌 계획 문서 동기화 전용이며, gx-dev의 `docs: [context] …` 커밋과 같은 층위의 예외다 (skill-routing.md에 명시). svn이면 커밋하지 않고 사용자에게 터미널에서 직접 반영하도록 안내한다.
