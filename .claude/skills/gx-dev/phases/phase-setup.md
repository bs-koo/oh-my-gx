...
**이어서 진행 시:**
- state.md에서 VCS_TYPE, GIT_PREFIX, PROJECT_ROOT, DEV_DIR, 베이스 브랜치, 프로젝트 타입, ARGS[0], flags, mode, preset을 복원. VCS_TYPE이 없으면 `"git"`으로 fallback. DEV_DIR이 없으면 브랜치명으로 재구성한다(브랜치명의 `/`를 `-`로 치환, Step 5의 DEV_DIR 설정 규칙과 동일).
- **레거시 모드 마이그레이션**: state.md의 `mode`가 `hotfix` 또는 `implement`(폐지된 구 모드)이면 `mode: light`로 갱신하고 안내한다: "구 모드({mode}) 세션을 라이트 모드로 전환하여 재개합니다." `hotfix`였으면 `preset: hotfix`를 함께 기록한다. 남은 Phase는 LIGHT 경로(`[light, complete]`)로 매핑한다 — 구 requirements가 completed이고 `${DEV_DIR}/prd.md`가 있으면 이를 ac.md 대용으로 사용하고(재작성하지 않음) phase-light Step 1(구현)부터, 구 implement가 in_progress였으면 phase-light Step 1부터 재개한다.
...
- **DEV_DIR 설정**: 브랜치명이 결정된 직후 `DEV_DIR`을 설정한다. branch-slug는 브랜치명에서 `/`를 `-`로 치환한다 (예: `feat/login` → `feat-login`). `DEV_DIR = .dev/{branch-slug}`. SVN인 경우 `DEV_DIR = .dev/trunk`. `mkdir -p ${DEV_DIR}`를 실행하여 디렉토리를 생성한다.
...
- mode, intent-source 기록 (의도 파싱 결과). LIGHT 긴급 프리셋이면 preset: hotfix도 기록
