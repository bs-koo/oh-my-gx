...
2. 각 state.md를 Read하여 `status: in_progress`인 것을 필터링한다.
...
`.dev/` 내 세션 전용 파일 패턴도 이 단계에서 함께 추가한다:
- `.dev/*/state.md` — 파이프라인 상태 (세션 전용)
- `.dev/*/diff.txt` — 변경사항 diff (git이 이미 추적)
- `.dev/*/self-check.md` — 자기점검 중간 결과 (review에서 상위 호환)

`.dev/` 자체는 `.gitignore`에 추가하지 않는다. prd.md, design.md, trust-ledger.md, codemap.md는 팀원이 공유할 수 있도록 git 추적 대상으로 유지한다.
