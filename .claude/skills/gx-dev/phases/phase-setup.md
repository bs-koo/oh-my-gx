...
- **DEV_DIR 설정**: 브랜치명(또는 svn 작업 slug)이 결정된 직후 `DEV_DIR`을 설정한다.
  ...
  - **svn**: 브랜치가 없으므로 git 브랜치명 생성과 **동일한 규칙**으로 작업 slug를 만든다 — ARGS에 `--slug <name>`이 있으면 그 값, 없고 ARGS[0]에 이슈 키(config `issueKey.pattern`)가 있으면 이슈 키, 둘 다 없으면 요청 타입+키워드로 `{type}-{description}`(최대 40자)을 만든다. slug는 `/`→`-` 치환 후 `[a-z0-9._-]`로 정규화하고 `/`·`..`를 제거한다. `DEV_DIR = .dev/{slug}` (고정 `.dev/trunk`가 아닌 **기능별 격리**). 결정한 slug를 `.dev/.active`에 기록한다(덮어쓰기) — 훅·라우팅·verify가 활성 작업의 state.md를 찾는 포인터.
