...
- **레거시 모드 마이그레이션**: state.md의 `mode`가 `normal`(구 명칭)이면 `mode: full`로 조용히 갱신하고 그대로 재개한다 (동작 동일, 명칭만 변경). `mode`가 `hotfix` 또는 `implement`(폐지된 구 모드)이면 `mode: light`로 갱신하고 안내한다: "구 모드({mode}) 세션을 라이트 모드로 전환하여 재개합니다." 남은 Phase는 LIGHT 경로(`[light, complete]`)로 매핑한다 — 구 requirements가 completed이고 `${DEV_DIR}/prd.md`가 있으면 이를 ac.md 대용으로 사용하고(재작성하지 않음) phase-light Step 1(구현)부터 재개한다. **ac.md도 prd.md도 없으면**(구 implement 세션 — 요구사항 기록이 없던 모드) phase-light **Step 0(AC 작성)부터** 재개하여 ARGS[0] 기반으로 ac.md를 먼저 생성한다 (맥락 없이 구현에 직행하지 않는다).
