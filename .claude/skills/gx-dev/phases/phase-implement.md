...
**방어 조건** (`--ralph`가 있어도 하나라도 해당하면 무시하고 "gx-ralph 전환을 건너뜁니다 — {사유}" 1줄 안내 후 "구현" 플로우로 직행. 의도 파싱이 정상 경로에서 이 조합을 막지만, `--resume`으로 구 state.md를 복원한 경우 등을 방어한다):
...
3. 의존성 힌트를 기반으로 **예비 배치**를 간이 위상 정렬로 구성한다:
   ...
   - 같은 파일을 수정하는 단계는 충돌을 막기 위해 서로 다른 순차 배치로 분리한다
...
5. AskUserQuestion으로 승인을 받는다:
   ...
   - **수정 요청 또는 Other** → 입력된 수정 내용으로 계획을 갱신 후 재제시. 사용자가 승인할 때까지 반복한다.
...
**건너뛰기 조건**: 설계서에 "구현 순서" 없는 경우 → Step 1과 함께 건너뜀.
...
for batch in batches:
  ...
  **단일 배치 + 단일 단계 (batches==1, steps==1)**:
    ...
    `Task(subagent_type="oh-my-gx:coder")` — prompt에 다음을 포함:
  ...
  **그 외**:
    if batch.parallel (단계 2개 이상):
      하나의 메시지에서 동시 Task(subagent_type="oh-my-gx:coder") 발행.
      각 coder에 배치 모드 prompt: 담당 단계 설계 + 담당 파일 목록 +
      이전 배치 결과 요약 + 병렬 안내 + 코드 맵 + PROJECT_ROOT + REFERENCES (있으면).
    else:
      단일 Task(subagent_type="oh-my-gx:coder") 배치 모드 호출.
  ...
  **배치 간 빌드 검증** (마지막 배치 제외):
    ...
    실패 시 → 에러 원인 특정 프로세스:
      ...
      5. 재빌드 실패: 사용자에게 실패 로그를 보고한 후 다음과 같이 진행 여부를 확인:
         ```
         AskUserQuestion(
           questions: [{
             header: "빌드 실패",
             question: "배치 빌드 검증에 재차 실패했습니다. 어떻게 진행할까요?",
             multiSelect: false,
             options: [
               { label: "재수정 시도", description: "coder에 추가 수정을 요청합니다" },
               { label: "중단", description: "현재 상태로 구현 Phase를 중단합니다" }
             ]
           }]
         )
         ```
...
- PRD의 "요구사항" + "수용 기준" 섹션만 (Context Slicing 규칙 참조: 자기점검 모드).
- "자기점검 단계이므로 CERTAIN 문제만 자동 수정 대상으로 취급할 것. QUESTION은 보고하되 수정하지 않고 phase-review로 이월한다."

**Step 6**: 결과 판단.
- **Critical이 있으면**: `Task(subagent_type="oh-my-gx:coder")` — Critical 항목 목록, qa-manager가 제시한 수정 방안, 코드 맵, 프로젝트 루트 경로를 prompt에 포함하여 자동 수정 (Context Slicing: coder 수정 모드). 수정 실패 시(coder가 해결하지 못했거나 수정 후에도 문제가 남는 경우) 미해결 Critical을 `SELF_CHECK_FINDINGS`에 `[Critical/미해결]`로 추가하고 phase-review로 이월한다. 자기점검은 1회만 시도하므로 재시도하지 않는다.
