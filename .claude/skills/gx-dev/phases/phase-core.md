...
**긴급 버그 수정 요청**("긴급/핫픽스" 키워드로 진입한 경우): AC를 버그 관점으로 구성한다 — AC-1은 재현 조건(현재 잘못된 동작), AC-2는 수정 후 기대 동작, 필요 시 AC-3에 회귀 방지 항목. 재현 조건을 파악할 정보가 부족하면 추측하지 말고 Step 0.5에서 사용자에게 확인한다.
...
- **coder 1회 디스패치** (그 외): `Task(subagent_type="oh-my-gx:coder")` — prompt에 다음을 포함:
  - ac.md 전체 (Context Slicing 규칙: coder 구현, 핵심 모드)
  - 코드 맵 (누적된 상태)
  - 프로젝트 루트 경로
...
- `"구현"` → ac.md를 Read하여 복원 후 Step 1부터 재실행. ac.md가 없으면 Step 0부터 재실행하여 ac.md를 먼저 생성한다.
