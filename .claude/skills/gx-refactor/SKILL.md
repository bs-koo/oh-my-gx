...
name: gx-refactor
version: 1.0.0
description: "REFACTOR 단계 - GREEN 상태 유지하며 중복 제거. 테스트 깨지면 즉시 롤백. 동작 변경 절대 금지."
argument-hint: ""
allowed-tools:
  - Read
  - Write
...
- 호출 시 GREEN 상태 검증:
  ...
  실패면: "REFACTOR는 GREEN 상태가 필요합니다. oh-my-gx:gx-green 먼저 호출하세요." 중단.
  ```
...
```
oh-my-gx:gx-refactor — REFACTOR 단계 진입.
...
- 에러 핸들링 추가
- 성능 최적화 (별도 작업)
- 인터페이스 시그니처 변경 (예외: 프로덕션 호출자가 0인 테스트 전용 메서드 제거는 허용)
...
✅ 올바름:
- `Skill("oh-my-gx:gx-red")` — 새 사이클 진입 (필요 시)
- `Skill("oh-my-gx:gx-verify")` — 사이클 완료 후 검증
