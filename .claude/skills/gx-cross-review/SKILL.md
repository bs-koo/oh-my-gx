...
```bash
mkdir -p ${DEV_DIR}
git diff $(git merge-base HEAD ${BASE_BRANCH})...HEAD > ${DIFF_FILE}
```
...
```bash
git diff $(git merge-base HEAD ${BASE_BRANCH})...HEAD --stat > ${DIFF_FILE}
echo "---" >> ${DIFF_FILE}
...
> **prompt 본문 작성 규칙** (셸 인용/길이 안전):
> - 본문에 백틱(`` ` ``), 이스케이프되지 않은 큰따옴표(`"`), `$` 시작 토큰은 가급적 사용하지 않는다. 코드 인용은 4칸 들여쓰기로 대체.
> - 합산 길이가 100KB(약 30,000~40,000 토큰)를 넘으면 위치 인자 길이 제한(특히 Windows ~32K)에 걸릴 수 있다. Step 2-1의 슬라이싱 임계(60,000 토큰)를 우선 적용해도 안전 마진을 확보한다.
...
```bash
node "${CODEX_COMPANION}" task "$(cat ${PROMPT_FILE})" \
  > ${RAW_FILE} 2>&1
```

- Bash 도구 호출 시 `timeout: 300000` 파라미터를 명시한다 (기본 2분으로는 부족).
- `--write` 플래그는 명시적으로 사용하지 않는다 (read-only 검증).
- `"$(cat ...)"`로 감싸 추가 expansion이 일어나지 않도록 하되, 본문의 백틱·`$VAR`·이스케이프되지 않은 따옴표는 prompt builder 단계에서 제거한다 (3a-1 안전 작성 규칙 참조).
...
```bash
node "${CODEX_COMPANION}" review --wait --base ${BASE_BRANCH} --scope ${SCOPE:-auto} > ${RAW_FILE}
```
- `--wait`: 동기 결과 수신.
- `--base`: 베이스 브랜치 명시 (Step 0-2 결과).
- `--scope`: `${SCOPE}`가 `stat`이면 그대로 전달, 그 외에는 `auto` 사용. (companion 사양: `auto|working-tree|branch`)
...
- **컨텍스트 초과**: Step 2-1의 슬라이싱으로 80,000 토큰 이하로 압축. 그래도 초과하면 사용자에게 `--scope stat` 사용 안내.
