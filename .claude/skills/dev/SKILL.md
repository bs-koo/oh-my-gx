...
- **product-owner (PRD 작성)**: ARGS[0] + 코드 맵 (패턴 스냅샷 포함) + 프로젝트 타입/구조 + DOMAIN_CONTEXT (있으면). **프로젝트 루트 경로를 전달하지 않는다** — PO는 소스 파일을 직접 Read하지 않고 코드 맵과 패턴 스냅샷만으로 PRD를 작성한다.
- **product-owner (인수 검증)**: PRD의 "요구사항" + "수용 기준" + diff 파일 경로 (`DIFF_FILE`) + 코드 맵
- **architect (설계)**: PRD 전체 + 코드 맵 (패턴 스냅샷 포함) + 프로젝트 타입/구조/컨벤션 + 프로젝트 루트 경로 + DOMAIN_CONTEXT (있으면) + REFERENCES (있으면). 패턴 스냅샷을 우선 참조하고, **추가 파일 Read는 핵심 파일 3개 이내**로 제한한다.
