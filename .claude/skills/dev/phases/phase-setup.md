...
**이어서 진행 시:**
- state.md에서 VCS_TYPE, GIT_PREFIX, PROJECT_ROOT, 베이스 브랜치, 프로젝트 타입, ARGS[0], flags를 복원. VCS_TYPE이 없으면 `"git"`으로 fallback.
...
5. **패턴 스냅샷 생성**: 핵심 파일 3~5개에서 구조적 정보를 발췌하여 코드 맵에 "패턴 스냅샷" 섹션을 추가한다. 이 스냅샷은 이후 agent(product-owner, architect)가 소스 파일을 직접 Read하지 않고도 기존 패턴을 파악할 수 있게 한다.

   **발췌 항목:**
   - **레이어 구조**: 진입점에서 데이터 계층까지의 호출 체인 (예: `Controller → Service → ServiceImpl → Mapper`)
   - **참조 파일별 구조**: 클래스 어노테이션, 주요 메서드 시그니처, 파라미터/리턴 패턴
   - **네이밍 컨벤션**: 파일명/클래스명에서 추출한 명명 규칙 (예: `{도메인}Controller.java`, `{도메인}_SQL.xml`)
   ...
   **스냅샷 형식:**
   ...
   #### 참조 파일: EnergyController.java
   - 클래스: @Controller, @RequestMapping("/energy")
   - 주요 메서드: getUsageList(), getAnalysisReport()
   - 파라미터 패턴: @ModelAttribute("searchVO") SearchVO
   - 리턴 패턴: "forward:/energy/usageList.do"
...
탐색은 **가볍게** — 파일 전체를 읽지 않고, 역할 파악에 필요한 최소한만 읽는다. 코드 맵에 등록하는 파일은 **최대 15개**로 제한한다 (핵심 ≤ 5, 참조 ≤ 7, 설정 ≤ 3). 패턴 스냅샷은 핵심 파일 중 **3~5개**에서만 발췌한다. 초과 시 관련도가 높은 파일을 우선한다.
...
- vcs-type, branch, base, project-type, project-root, args, flags 기록
