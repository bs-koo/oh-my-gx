...
## 0.7 팀 모드 초기화 (mode=team일 때만)

`mode: normal`이면 이 단계를 건너뛴다.

1. **팀 생성**:
   ```
   TeamCreate({ team_name: "dev-{branch}-{timestamp}", description: "[작업 설명]" })
   ```

2. **공유 메모리 초기화**: 다음 3개 파일을 `.dev/`에 Write한다:
   - `TEAM_PLAN.md` — 초기 구조:
     ```markdown
     # Team Plan
     ## 팀 구성
     - 팀장: architect (opus)
     - coder 목록: (implement에서 배정)
     ## 태스크 분배
     (설계서 기반으로 implement에서 채움)
     ## 결정사항
     (진행 중 기록)
     ```
   - `TEAM_PROGRESS.md` — 초기 구조:
     ```markdown
     # Team Progress
     | 팀원 | 태스크 | 상태 | 시작 | 완료 |
     |------|--------|------|------|------|
     ```
   - `TEAM_FINDINGS.md` — 초기 구조:
     ```markdown
     # Team Findings
     ## 발견사항
     ## DEAD_ENDS
...
3. **팀장(architect) 스폰**:
   ```
   Task({ team_name, name: "팀장", subagent_type: "architect", model: "opus" })
   ```
   팀장 프롬프트에 포함:
   - 역할: 태스크 분배, 진행 모니터링, 품질 검증
   - 금지: 직접 코드 작성
   - 공유 메모리 3개 파일 경로
   - 설계서, PRD, 코드 맵

4. **state.md에 team_name 기록**:
   ```yaml
   team_name: dev-{branch}-{timestamp}
   ```
