...
- 프로젝트 루트 경로 (추가 파일 Read가 필요한 경우에만 사용)
- "코드 맵의 패턴 스냅샷을 참조하여 기존 구현 패턴(레이어 구조, 네이밍, 에러 처리 방식 등)을 파악하라. 새 설계가 기존 패턴과 일관되도록 할 것. 패턴 스냅샷만으로 부족한 경우에만 추가 파일을 Read하되, **핵심 파일 3개 이내**로 제한하라."
- "설계"로 동작할 것
