---
name: gx-lens
description: 코드에서 비즈니스 정책을 탐지하여 영향도를 분석한다. "정책 확인", "영향도" 시 사용.
...
이 스킬의 파일들은 프로젝트 루트의 `.claude/skills/gx-lens/` 하위에 위치한다.
...
ARGS[0]이 없으면 다음을 응답:
"탐지할 정책을 자연어로 설명해주세요. 예: `/gx-lens 결제 한도 정책 확인해줘`"

ARGS[0]이 `--`로 시작하면 다음을 응답:
"쿼리는 자연어로 입력해주세요. 옵션은 쿼리 뒤에 추가합니다. 예: `/gx-lens 결제 한도 정책 --detail`"
...
```
Read(`<프로젝트 루트>/.claude/skills/gx-lens/phases/phase-<name>.md`)
```
