...
아래 `<archify_command>`는 실제로 실행되는 명령(리스트)의 자리표시자다. `render_archify.py`는 `--archify-command`를 생략하면 `ensure_archify()`로 이 값을 스스로 찾는다 — **정상 경로는 이 인자를 생략한다.** 호출자가 4번(`ensure_archify()`)에서 확정한 `command`를 꺼내 5번에 다시 넘길 필요가 없다.

과거에는 이 값을 호출자가 `--archify-command`에 **있는 그대로(verbatim)** 전달하도록 했다 — 경로 형태를 바꾸거나(Git Bash의 POSIX 스타일 경로 `/c/Users/...`로 재조립하는 등, node가 이를 상대경로로 오인해 조용히 폴백된다: T6에서 실측) 손으로 다시 조립하지 않는다는 취지였다. Python API(`render_archify()`)를 직접 호출하면 `command`(리스트)를 그대로 넘기면 되지만, CLI(`render_archify.py`)의 `--archify-command`는 문자열 인자 하나이므로 리스트를 문자열로 바꿔야 했다. `shlex.split(..., posix=False)`와 `subprocess.list2cmdline()`의 왕복은 항등이 아니다 — `command[0]`이 `C:\Program Files\nodejs\node.EXE`처럼 공백을 포함하면 `list2cmdline`이 붙인 인용부호를 `posix=False` 역직렬화가 벗기지 않아 실행 파일 경로가 쪼개진다(2026-09-18 최종 리뷰 I2). `json.dumps(command)` 직렬화는 이 Python 수준의 왕복 문제는 해결하지만, 그 문자열이 **실제로는 셸을 거쳐** 전달된다는 점은 해결하지 못한다 — 이 스킬의 실행자는 Bash 도구를 쓰는 에이전트이므로, JSON 배열 문자열이 git-bash에서는 이스케이프된 백슬래시가 손실돼 `json.loads`가 실패하고, PowerShell에서는 큰따옴표가 통째로 사라진다(T15). `--archify-command`가 선택 인자가 된 이유가 이것이다 — 정상 경로는 셸을 통과하는 명령 문자열 자체를 없애 이 문제를 원천에서 제거한다.

`--archify-command`를 명시해야 하는 경우(테스트, 특정 Archify 빌드로 override)에는 Python API에 `command`(리스트)를 직접 넘기는 방식을 우선한다. CLI로 명시해야 한다면 `json.dumps(command)`로 직렬화하되, **이 인자를 셸(Bash 도구·PowerShell 등)을 거쳐 넘기면 깨질 수 있다는 것을 알고 쓴다** — 셸을 거치지 않는 호출(Python에서 `subprocess.run([...], shell=False)`로 직접 넘기는 경로)에서만 이 직렬화가 안전하다.
