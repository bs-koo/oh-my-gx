...
아래 `<archify_command>`는 `ensure_archify()`·`detect_backend()`가 돌려준 `command` 값을 **있는 그대로(verbatim)** 쓴다 — 경로 형태를 바꾸거나 손으로 다시 조립하지 않는다. 그 값은 `shutil.which`나 홈 디렉터리 후보에서 나온 Windows 네이티브 경로다; Git Bash의 POSIX 스타일 경로(`/c/Users/...`)로 바꿔 넘기면 node가 이를 상대경로로 오인해 조용히 폴백된다(T6에서 실측).

Python API(`render_archify()`)를 직접 호출하면 `command`(리스트)를 그대로 넘기면 된다. CLI(`render_archify.py`)의 `--archify-command`는 문자열 인자 하나이므로 리스트를 문자열로 바꿔야 하는데, `shlex.split(..., posix=False)`와 `subprocess.list2cmdline()`의 왕복은 항등이 아니다 — `command[0]`이 `C:\Program Files\nodejs\node.EXE`처럼 공백을 포함하면 `list2cmdline`이 붙인 인용부호를 `posix=False` 역직렬화가 벗기지 않아 실행 파일 경로가 쪼개지고, exit code 0에 "액세스가 거부되었습니다"만 남아 원인 추적도 어렵다(2026-09-18 최종 리뷰 I2). CLI를 쓸 때는 `json.dumps(command)`로 직렬화한다 — `render_archify.py`가 `--archify-command` 값이 JSON 배열이면 그대로 argv로 받아들이므로 공백 포함 경로도 항등으로 왕복한다.
