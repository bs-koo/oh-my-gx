...
- 도메인 컨텍스트 (DOMAIN_CONTEXT가 있으면 용어·아키텍처만): 기존 용어를 그대로 쓰고, `architecture.md`의 레이어·의존 규칙과 다른 설계는 "의존성 및 영향도" 절 안에 "기존 구조와의 차이" 소제목으로 명시할 것 (architect 출력 포맷에 새 절을 만들지 않는다)
