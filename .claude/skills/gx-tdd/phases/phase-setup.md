...
### `--resume` 플래그가 있는 경우
1. state.md 탐색을 위해 경로를 계산한다 (VCS 판별: `git rev-parse --is-inside-work-tree` 성공 시 git, 실패하고 `svn info` 성공 시 svn):
...
**svn인 경우** → `.dev` 산출물(state.md, diff.txt 등)이 `svn status`에 노출되거나 실수로 커밋되지 않도록 `svn:ignore` 속성에 `.dev`를 추가한다. `svn propget svn:ignore .`로 기존 값을 읽어 `.dev`가 없으면 추가한 뒤 `svn propset svn:ignore -F <목록파일> .`로 설정한다 (기존 ignore 패턴은 보존, `-F`는 목록 파일에서 읽음). 처리 후 Step 7로 진행한다.
