...
`standard`로 결정되면 안내한다: "표준 프로파일 — 에이전트를 frontmatter 모델대로 디스패치합니다 (architect·coder·design-critic·test-architect·quality-reviewer 등 opus 에이전트는 세션 모델과 무관하게 opus로 실행). 세션 모델을 sonnet으로 낮춰 절감하려면 표준이 아니라 eco를 쓰세요 — 표준은 이 opus 에이전트들을 그대로 유지하므로 절감 효과가 제한적입니다."
...
4. **브랜치명을 `작업` 열에서 생성한다.** `{type}/{작업 열을 슬러그화한 값}` 형식이며 type은 AskUserQuestion으로 확인한다 (기본값 `feat`). svn이면 브랜치 대신 `.dev/{slug}/`의 slug로 쓴다.
...
8. **중복 착수 확인** (읽기만 한다 — 표를 여기서 고치지 않는다):
...
2. **전용 커밋**: `.dev/plan.md`만 스테이징해 `docs: [plan] {ID} 착수` 메시지로 커밋하고 **현재 작업 브랜치로** push한다. 베이스 브랜치로 보내지 않는다 — 브랜치 보호가 걸린 저장소에서 거부되고, phase-complete의 완료 갱신과 층위가 어긋난다. push 실패는 경고만 남기고 진행한다 (착수 기록은 조율 수단이지 게이트가 아니다).
