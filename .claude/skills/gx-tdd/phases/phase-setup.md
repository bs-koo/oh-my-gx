...
**svn인 경우** → 격리 브랜치를 만들지 않는다. SVN은 trunk에서 직접 작업하며, `svn update`로 최신 상태만 동기화한다. `DEV_DIR = .dev/trunk/`로 설정하고 `mkdir -p ${DEV_DIR}`를 실행한다. 완료 후 프로젝트 타입, 작업 경로를 사용자에게 보고하고 Step 7로 진행한다 (Step 6, 6.5는 git 전용).
