...
commit/PR로 이어지는 이 Phase의 **모든 진입**(전체 파이프라인, `--phase complete` 단독, 수동 수정 후 재호출)에서 수행한다:

1. state.md를 확인한다. 다음 중 하나면 **TDD 미이행 가능성**으로 판정한다:
   - `pipeline: gx-tdd` 필드가 없다 (gx-dev 이력이거나 알 수 없는 state — phases 기록을 신뢰할 수 없다). **예외**: `pipeline: gx-ralph`이고 `origin: gx-tdd`이면 RGR 트리오로 구현된 무인 루프 산출물이므로 **이행으로 인정**한다 (루프 종료 후 복귀 경로의 오탐 방지).
