...
verify 게이트는 테스트 0 failures·빌드 성공에 더해, phase-review mechanical gate가 기록한 `warnings-baseline` 대비 **신규 경고**도 차단한다 (기존 경고는 허용).
...
> 주의: `oh-my-gx:gx-commit`은 gx-dev와 공유하는 무수정 스킬이므로 verify 게이트(테스트 실행 증거 수집)를 포함하지 않는다.
