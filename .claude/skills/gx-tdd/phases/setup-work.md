...
작업 ID를 확정한 실행, 또는 작업 ID는 없지만 `.dev/plan.md`가 존재하는 실행에서 Read한다 (후자는 "계획이 있으나 작업 ID가 지정되지 않은 경우" 소절만 수행한다). 세 절은 phase-setup의 3.0.5(작업 계획 참조) · 5.5(착수 기록) · 7(작업 계획 되돌림) 자리에서 각각 호출된다. 원문의 Step 번호 인용(3.0.5, 5.5, Step 7)은 phase-setup의 해당 포인터 위치를 가리킨다.
