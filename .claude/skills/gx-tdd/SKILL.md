...
| `긴급`, `핫픽스`, `급한`, `빨리 고쳐`, `버그 수정만` | LIGHT | "로그인 버그 긴급 수정해줘" (AC를 재현 조건 관점의 G-W-T로 작성) |
| `구현만`, `라이트`, `가볍게` | LIGHT | "알림 임계값 검증, TDD로 구현만 해줘" |
| `설계만`, `PRD만`, `리뷰만`, `커밋만` | PHASE(해당) | "설계만 해줘" |
| `{branch}에서`, `{branch} 기반`, `{branch} 브랜치` | BASE 추출 | "develop 브랜치 기반으로 작업해줘" |

PHASE 매핑: `PRD만`/`요구사항만` → `--phase requirements`, `설계만` → `--phase design`, `리뷰만` → `--phase review`, `커밋만`/`PR만` → `--phase complete`. implement phase 단독 실행은 자연어 매핑 없이 `--phase implement` 플래그 전용이다 (자연어 "구현만"은 LIGHT로 라우팅 — gx-dev와 동일한 정렬).
...
위 자동 판정 패턴에서 모드(STATUS/RESUME/LIGHT/PHASE)가 결정되지 않으면 — 즉, 일반적인 기능 요청이면 — **반드시** AskUserQuestion으로 모드를 확인한다. 오케스트레이터가 임의로 모드를 판정하지 않는다.
...
AskUserQuestion(
  questions: [{
    ...
    options: [
      ...
      { label: "라이트", description: "AC(G-W-T) 확인 → RGR → verify → PR. 소형 변경용 — design/정식 review 생략, RGR·verify·긴급 감사는 유지" }
    ],
    multiSelect: false
  }]
)
...
> **gx-dev의 라이트와 다른 점**: gx-tdd의 라이트는 **RGR 사이클·verify 게이트·G-W-T 게이트·긴급 보안 감사를 전부 유지**하는 경량 경로다 (Iron Law는 모드와 무관하게 불변). 생략되는 것은 design(testability 평가)과 정식 review뿐이며, AC 작성 주체가 product-owner에서 오케스트레이터로 바뀐다. 테스트 작성 강제 없이 즉시 구현하려면 `oh-my-gx:gx-dev`의 라이트 모드를 사용한다.
...
- `--light`: 라이트 모드 (AC(G-W-T) 확인 → RGR → verify → PR)
...
| requirements | phase-requirements.md | product-owner (LIGHT 모드는 inline — 오케스트레이터 직접 ac.md) | **AC = Given-When-Then 강제** (G-W-T 게이트 — 오케스트레이터 직접 검증) | Yes (max 1) |
...
- **implement**: 정상 모드와 동일하게 RGR 사이클 수행. 단, design.md 부재로 red-writer/green-coder에 ac.md의 AC만 전달. 사이클 종료 후 H1~H4 (긴급 보안 감사: CRITICAL/HIGH만) 실행.
...
- 긴급 버그 수정 요청("긴급/핫픽스" 키워드, 레거시 `--hotfix`)도 이 경로다 — AC를 재현 조건 관점의 G-W-T로 작성한다.

**Iron Law 유지 (light여도)**:
- Iron Law 1 (실패 테스트 우선): RGR 사이클이 light에서도 강제됨
...
> Phase를 건너뛸 수 있는 조건은 LIGHT 모드(사용자가 명시적으로 선택한 경량 경로 — RGR·verify·G-W-T 게이트는 유지), `--phase` 플래그, 그리고 phase-implement Step 0.7의 "구현 방식 확인"에서 사용자가 **ralph 무인 루프**를 선택한 경우(gx-ralph 호출 후 종료 — 이후 Phase는 루프 종료 후 사용자 복귀로 재개)뿐이다.
...
if light (LIGHT 모드):
    PHASES = [setup, requirements, implement, complete]   # requirements는 light 분기 (오케스트레이터 직접 ac.md)
elif --phase 지정:
...
for phase in PHASES:
    ...
    if phase == "implement" and light and not exists("${DEV_DIR}/ac.md"):
        → phase-requirements(light 분기)부터 실행
    if phase == "implement" and not light and not exists("${DEV_DIR}/prd.md"):
        → phase-requirements부터 실행
    if phase == "implement" and not light and not exists("${DEV_DIR}/design.md"):
        → phase-design부터 실행

    # 2a-1. testability 게이트 (Iron Law) — design.md에 testability 섹션 필수
    if phase == "implement" and not light:
...
**위험 수용 기록 규약**: 파이프라인의 모든 위험 수용(테스트 미검증 리뷰, 미해결 Critical 수용, TDD 미이행 완료 실행, 신규 경고 수용, G-W-T 제외, light 긴급 감사 수용 등)은 `### 위험 수용` 섹션에 `- [{항목명}] {사유} ({phase/step})` 형식으로 기록한다. Write 권한이 없는 스킬(gx-verify)은 보고만 하고 **오케스트레이터가 기록**한다.
...
**생성**:
- FULL 모드: phase-review Step 3의 security-auditor 통합 감사 완료 시 생성.
- LIGHT 모드: phase-implement Step H1~H4의 긴급 보안 감사 완료 시 생성 (`### Light 긴급 감사` 섹션).
...
#### EXECUTION (RED-GREEN-REFACTOR 순차; red-writer만 코드 격리)
- **red-writer (RED)** ← 신규: AC (Given-When-Then 시나리오 — LIGHT 모드이면 ac.md의 AC) + 설계서의 testability 섹션 (LIGHT 모드는 없음 — 기존 테스트 스타일만 근거) + 기존 테스트 스타일 + 프로젝트 루트 경로. **기존 프로덕션 코드는 절대 포함하지 않는다** (격리 — 위반 여부는 verify_red가 "참조한 파일" 자기신고로 검증). "테스트만 작성. 프로덕션 코드 작성 금지" 지시.
...
- `--light`(레거시 `--hotfix` 포함)와 `--phase`는 **동시 사용 불가**. 둘 다 있으면: "`--light`/`--hotfix`와 `--phase`는 동시에 사용할 수 없습니다." 에러 후 중단.
- `--resume`과 `--phase`, `--light`, `--hotfix`, `--status`는 **동시 사용 불가**. 함께 있으면: "`--resume`은 다른 모드 플래그와 동시에 사용할 수 없습니다." 에러 후 중단.
