...
> **gx-dev와 달리 라이트(light: AC 확인 → 구현 → Gate, 테스트 작성 강제 없음) 경량 모드는 제공하지 않는다.** TDD 파이프라인의 Iron Law 1(실패 테스트 우선)과 정면 충돌하기 때문이다. 설계를 건너뛰고 빠르게 진행하려면 "긴급 수정"(hotfix: 경량 PRD + RGR + verify)을 사용하고, 테스트 강제 없이 즉시 구현하려면 `oh-my-gx:gx-dev`의 라이트 모드를 사용한다.
...
> Phase를 건너뛸 수 있는 조건은 `--hotfix` 모드, `--phase` 플래그, 그리고 phase-implement Step 0.7의 "구현 방식 확인"에서 사용자가 **ralph 무인 루프**를 선택한 경우(gx-ralph 호출 후 종료 — 이후 Phase는 루프 종료 후 사용자 복귀로 재개)뿐이다.
