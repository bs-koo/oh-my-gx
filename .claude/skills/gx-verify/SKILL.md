...
**복수 타입이 감지되면 전부 실행한다.** `detect` 매칭이 2개 이상이면 각 타입의 test·build를 모두 돌리고, **하나라도 실패하면 게이트를 차단**한다. 한쪽만 골라 실행하면 나머지 레이어(흔히 프론트엔드)가 검증 없이 통과하는데, 그건 verify 게이트가 존재하는 이유를 정면으로 무효화한다. 경고 카운트는 타입별 `warningPattern`으로 각각 세어 합산한다. 실행 순서는 config 등록 순서를 따른다.

복합 명령 하나로 등록된 경우(`"test": "A && B"`)는 그 자체가 단일 명령이므로 그대로 실행하면 된다 — 등록 방식은 `/gx-setup`이 안내한다.
