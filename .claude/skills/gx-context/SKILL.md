...
2. 답변을 평가한다:
   - **명확하고 충분**: 다음 프레임으로 이동
   - **모호하거나 불완전**: 파고들기 질문을 생성하여 현재 위치에 이어서 진행 (파고들기 질문에도 동일 규칙(권장 답변 또는 예시 후보 + Other)이 적용된다. 아래 기준 참고)
...
1. 현재 프레임을 `AskUserQuestion(questions: [질문 1개])`으로 제시한다. **확인형이든 개방형이든 권장 답변(문서에서 추출된 내용 또는 합리적 추정)을 options 첫 번째에 `(Recommended)` 라벨로 반드시 제시한다.**
   - **문서에서 추출된 항목** (확인형): "문서에서 '{추출 내용}'으로 파악했습니다. 맞나요?"
     ...
     ```
   - **문서에서 파악 안 된 항목** (개방형): 모드 B-2와 동일하게 권장 답변 + Other + 모르겠음
     ```
     options: [
...
2. 답변을 평가한다. 확인형의 "수정 필요" + Other 입력은 개방형 응답과 동일한 평가 규칙을 적용한다:
   ...
   - **모호하거나 불완전**: 파고들기 질문을 생성하여 현재 위치에 이어서 진행 (파고들기 질문에도 동일 규칙(권장 답변 또는 예시 후보 + Other)이 적용된다)
   - **결정 의존성 발견**: 이 답변이 후속 프레임의 전제 또는 형태를 바꾸면, 영향받는 프레임을 재구성하거나 종속 질문을 추가한다.
   - **코드베이스로 답 가능**: 직접 탐색 후 확인하고 다음 프레임으로 이동
   - **"모르겠음" 선택**: ❓로 기록하고 다음 프레임으로 이동
...
1. 기존 원장의 모든 ID와 입력에서 유효하고 고유한 ID를 먼저 예약하여 `RESERVED_LEDGER_IDS` 집합을 만든다.
2. 각 후보를 모든 기존 행과 비교한다. 입력 ID가 유효하고 기존 행의 ID와 같으면 같은 항목으로 본다.
3. ID가 없으면 공백·문장부호를 제거한 핵심 문장을 모든 기존 행에서 비교하여 같은 항목을 찾는다.
4. 기존 원장에 없는 유효하고 고유한 입력 ID는 그대로 사용하며, ID는 `FR-N`·`NFR-N` 형식이어야 한다. 예를 들어 기존 원장에 `NFR-2`가 없고 입력에서 한 번만 나오면 `NFR-2`는 그대로 유지한다.
5. ID가 없거나 유효하지 않거나 입력 안에서 중복된 새 항목에 자동 ID를 할당할 때는 예약 집합의 유형별 최댓값 다음 번호를 사용하고 할당 즉시 `RESERVED_LEDGER_IDS`에 추가한다. 삭제된 번호를 재사용하지 않는다. 예: 기존 `FR-1` + 입력 `FR-2` + ID 없는 FR → `FR-2`, `FR-3`.
6. 기존 입력에서 사라진 `⬜` 행은 변경 제안에 포함하고 사용자가 제거를 승인한 경우에만 `🚫`로 바꾼다. `✅` 행은 자동 폐기하지 않는다.
7. Edit 전에 제안 표의 ID 중복을 검사하고, 완성된 제안 표를 변경 전 스냅샷의 모든 기존 행과 비교한다. 기존 `✅` 행의 ID·AC·상태·PR이 변경되면 Edit하지 않는다.
8. 매칭과 번호 할당이 끝나면 `LEDGER_REQUIREMENTS`의 각 객체를 최종 `{ id, type, text, ac, status, pr }`로 갱신한다.
9. Edit으로 기존 행을 수정하고 새 행을 표 끝에 추가한다. 기존 파일 전체를 Write로 덮어쓰지 않는다.
10. 저장한 표를 다시 읽어 같은 객체 형태로 정규화하고 ID 중복, 빈 요구사항, 허용되지 않은 상태, 기존 `✅` 행의 ID·AC·상태·PR 변경이 없는지 확인한다. 저장한 표의 행과 `LEDGER_REQUIREMENTS`가 일치하는지 같은 ID끼리 대조하고, 각 객체의 id·type·text·ac·status·pr 중 하나라도 일치하지 않으면 C-5로 진행하지 않는다.
...
5. 영속 cursor를 명령 인자로 넣기 전에 다음 형식과 실제 값을 엄격히 검증한다.
...
6. 검증 후 조회가 시작되기 전에 상한을 한 번만 캡처한다. git의 `git rev-parse HEAD`는 `CANDIDATE_GIT_HEAD`, svn의 `svn info --show-item revision`은 `CANDIDATE_SVN_REVISION`, gh가 있으면 현재 UTC 시각은 `CANDIDATE_PR_SYNC_AT`에 저장한다. 각 후보 캡처 명령과 후보 값 검증이 성공해야 하며, 아직 status.md에는 쓰지 않는다. 이후 모든 조회는 이 후보 상한을 사용한다.
...
3. **svn 조회**: `SYNC_SVN_REVISION`이 `-`가 아니면 `SVN_FROM_REVISION = SYNC_SVN_REVISION + 1`로 계산하여 `svn log --xml -r ${SVN_FROM_REVISION}:${CANDIDATE_SVN_REVISION}`을 조회한다. 하한이 상한보다 크면 성공한 빈 범위로 처리한다. 초기 조회는 각 pending ID로 `svn log --xml -r 1:${CANDIDATE_SVN_REVISION} --search <ID>`를 수행하고, 반환된 각 logentry의 메시지에서 exact token을 재검증한다. 설명 키워드는 `svn log --xml -l 100 -r ${CANDIDATE_SVN_REVISION}:1`에서 보조 검색한다.
4. **PR 조회**: gh가 있으면 저장소를 확인한 뒤 `gh api --paginate`로 `repos/{owner}/{repo}/pulls?state=closed&sort=updated&direction=desc&per_page=100`의 모든 page를 끝까지 읽는다. 각 응답에서 number·title·body·html_url·merged_at을 읽고, merged_at이 null이 아닌 PR만 남긴다. cursor가 있으면 `start = SYNC_PR_MERGED_AT`, 초기 조회면 하한 없음으로 두고 `start <= merged_at < candidate` (`candidate = CANDIDATE_PR_SYNC_AT`) 범위만 분석한다. 모든 page의 명령·JSON 파싱·필드 처리가 성공해야 PR 분석 성공이며, 한 page라도 실패하면 PR `분석 실패`다.
5. **exact ID 매칭**: git `%B`, SVN 메시지, PR 제목·본문에서 pending ID를 `(^|[^A-Za-z0-9-])<ID>($|[^A-Za-z0-9-])` 경계로 재검증한다. 따라서 `FR-1`은 `NFR-1`이나 `FR-10`과 일치하지 않는다. exact ID를 우선 매칭하고, ID가 없을 때만 설명 키워드 일치를 후보로 제시한다.
...
- 수정일 갱신
- 사용자가 `전체 반영`, `조정 후 반영`, `건너뛰기` 중 하나를 확정하고 모든 분석 명령과 PR의 모든 page 처리가 성공한 경우에만 승인한 행 변경과 gx-sync 블록 갱신을 같은 Edit으로 수행한다. 활성 소스의 cursor에는 캡처해 둔 `CANDIDATE_GIT_HEAD`, `CANDIDATE_SVN_REVISION`, `CANDIDATE_PR_SYNC_AT`을 기록하고, 사용하지 않은 소스의 기존 값은 유지한다.
- 질문 중단, cursor·후보 검증 실패, 파일 Edit 실패, 분석·명령·파싱 실패, PR page 일부 실패 시 기존 cursor를 유지한다.
