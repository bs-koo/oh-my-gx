   ...
   AskUserQuestion(
     questions: [{
       ...
       options: [
         ...
         { label: "수정 필요", description: "Other로 이동해서 수정할 프레임 번호와 새 답변을 입력해주세요 (예: '3) 마이그레이션 리스크 누적')" }
       ],
       multiSelect: false
     }]
   )
   ```
   "맞습니다"면 B-4로 진행. "수정 필요"면 사용자가 Other에 입력한 `프레임 번호: 새 답변` 패턴을 파싱하여 해당 프레임을 재질문 큐에 추가한 뒤 루프 재진입한다. 입력이 패턴과 일치하지 않거나 번호를 식별할 수 없으면, 어떤 프레임을 수정할지 선택형 질문으로 다시 묻는다.
...
3. 모든 프레임이 해소되면 **공유 이해 확인(align)** 단계를 거친다. B-2의 align 절차와 동일하게 "프레임 → 답변" 요약을 `{ label: "맞습니다 (Recommended)" }` / `{ label: "수정 필요" }` 형태로 확인한다. "수정 필요" 시 `프레임 번호: 새 답변` 패턴 파싱 및 패턴 불일치 시 선택형 재질문 규칙도 B-2와 동일. "맞습니다"면 C-4로 진행. "수정 필요"면 해당 프레임을 재질문하여 루프 재진입한다.
