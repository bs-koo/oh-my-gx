...
1. 현재 프레임을 `AskUserQuestion(questions: [질문 1개])`으로 제시한다:
   - **문서에서 추출된 항목**: "문서에서 '{추출 내용}'으로 파악했습니다. 맞나요? 보완할 내용이 있으면 알려주세요." (확인형)
   - **문서에서 파악 안 된 항목**: 모드 B-2와 동일한 개방형 질문 (`options: [{label: "Other로 입력"}, {label: "모르겠음"}]`)

2. 답변을 평가한다:
   - **명확하고 충분 (또는 확인형에서 동의)**: 다음 프레임으로 이동
