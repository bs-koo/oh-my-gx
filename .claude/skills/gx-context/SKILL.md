...
1. 현재 프레임의 질문을 `AskUserQuestion(questions: [질문 1개])`으로 제시한다. **권장 답변(코드베이스/도메인 맥락에서 가장 가능성 높은 답 또는 합리적 추정)을 options 첫 번째에 `(Recommended)` 라벨로 반드시 제시**한다. `options` 구조:
...
2. 답변을 평가한다:
   - **명확하고 충분**: 다음 프레임으로 이동
   - **모호하거나 불완전**: 파고들기 질문을 생성하여 현재 위치에 이어서 진행 (아래 기준 참고)
   ...
   AskUserQuestion(
     questions: [{
       ...
       options: [
         ...
         { label: "수정 필요", description: "Other로 이동해서 수정할 항목과 내용을 입력해주세요" }
       ],
       multiSelect: false
     }]
   )
   ```
   "맞습니다"면 B-4로 진행. "수정 필요"면 해당 프레임을 재질문하여 루프 재진입한다.
...
2. 답변을 평가한다. 확인형의 "수정 필요" + Other 입력은 개방형 응답과 동일한 평가 규칙을 적용한다:
   ...
   - **모호하거나 불완전**: 파고들기 질문을 생성하여 현재 위치에 이어서 진행
   - **결정 의존성 발견**: 이 답변이 후속 프레임의 전제 또는 형태를 바꾸면, 영향받는 프레임을 재구성하거나 종속 질문을 추가한다.
   - **코드베이스로 답 가능**: 직접 탐색 후 확인하고 다음 프레임으로 이동
   - **"모르겠음" 선택**: ❓로 기록하고 다음 프레임으로 이동

3. 모든 프레임이 해소되면 **공유 이해 확인(align)** 단계를 거친다. B-2의 align 절차와 동일하게 "프레임 → 답변" 요약을 `{ label: "맞습니다 (Recommended)" }` / `{ label: "수정 필요" }` 형태로 확인한다. "맞습니다"면 C-4로 진행. "수정 필요"면 해당 프레임을 재질문하여 루프 재진입한다.
