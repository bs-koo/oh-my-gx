...
아래 5개 질문을 하나의 AskUserQuestion 호출에 `questions` 배열로 담아 한 번에 묻는다. 각 질문은 자유 입력이 필요하므로 `options`에는 "Other로 입력" / "모르겠음" 2개를 제공한다. 사용자는 Other로 이동해서 실제 내용을 자연어로 입력한다.
...
AskUserQuestion(
  questions: [
    ...
    {
      header: "담당",
      question: "이 도메인의 담당 PM/PO와 주요 개발자는?",
      multiSelect: false,
      options: [
        { label: "Other로 입력", description: "Other로 이동해서 자연어로 답변을 입력해주세요" },
        { label: "모르겠음", description: "아직 불명확합니다" }
...
추출 결과를 보여준 뒤, 아래 5개 항목 중 **문서에서 충분히 파악되지 않은 항목**만 추려서 하나의 AskUserQuestion 호출에 `questions` 배열로 담아 묻는다. 이미 문서에서 명확히 파악된 항목은 `questions` 배열에서 제외한다. 각 질문은 모드 B-2와 동일하게 `options: [{label: "Other로 입력"}, {label: "모르겠음"}]` 구조를 사용하고, description으로 "Other로 이동해서 자연어로 답변을 입력해주세요"라고 명시한다.
