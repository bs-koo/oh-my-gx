...
1. 각 후보를 모든 기존 행과 비교한다. 입력 ID가 유효하고 기존 행의 ID와 같으면 같은 항목으로 본다.
2. ID가 없으면 공백·문장부호를 제거한 핵심 문장을 모든 기존 행에서 비교하여 같은 항목을 찾는다.
3. 기존 원장에 없는 유효하고 고유한 입력 ID는 그대로 사용하며, ID는 `FR-N`·`NFR-N` 형식이어야 한다. 예를 들어 기존 원장에 `NFR-2`가 없고 입력에서 한 번만 나오면 `NFR-2`는 그대로 유지한다.
4. ID가 없거나 유효하지 않거나 입력 안에서 중복된 새 항목만 유형별 기존 최댓값 다음 번호를 받는다. 삭제된 번호를 재사용하지 않는다.
5. 기존 입력에서 사라진 `⬜` 행은 변경 제안에 포함하고 사용자가 제거를 승인한 경우에만 `🚫`로 바꾼다. `✅` 행은 자동 폐기하지 않는다.
6. Edit 전에 완성된 제안 표를 변경 전 스냅샷의 모든 기존 행과 비교한다. 기존 `✅` 행의 ID·AC·상태·PR이 변경되면 Edit하지 않는다.
7. 매칭과 번호 할당이 끝나면 `LEDGER_REQUIREMENTS`의 각 객체를 최종 `{ id, type, text, ac, status, pr }`로 갱신한다.
8. Edit으로 기존 행을 수정하고 새 행을 표 끝에 추가한다. 기존 파일 전체를 Write로 덮어쓰지 않는다.
9. 저장한 표를 다시 읽어 같은 객체 형태로 정규화하고 ID 중복, 빈 요구사항, 허용되지 않은 상태, 기존 `✅` 행의 ID·AC·상태·PR 변경이 없는지 확인한다. 저장한 표의 행과 `LEDGER_REQUIREMENTS`가 일치하는지 같은 ID끼리 대조하고, 각 객체의 id·type·text·ac·status·pr 중 하나라도 일치하지 않으면 C-5로 진행하지 않는다.
