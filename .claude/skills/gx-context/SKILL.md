...
1. 현재 프레임의 질문을 `AskUserQuestion(questions: [질문 1개])`으로 제시한다. `options`에는 아래 2개를 제공한다:
...
1. 현재 프레임을 `AskUserQuestion(questions: [질문 1개])`으로 제시한다:
   - **문서에서 추출된 항목** (확인형): "문서에서 '{추출 내용}'으로 파악했습니다. 맞나요?"
     ```
     options: [
       { label: "맞습니다", description: "다음 항목으로 이동" },
       { label: "수정 필요", description: "Other로 이동해서 수정/보완 내용을 자연어로 입력해주세요" }
     ]
     ```
   - **문서에서 파악 안 된 항목** (개방형): 모드 B-2와 동일하게 직접 질문
     ```
     options: [
       { label: "Other로 입력", description: "Other로 이동해서 자연어로 답변을 입력해주세요" },
       { label: "모르겠음", description: "아직 불명확합니다" }
...
2. 답변을 평가한다:
   - **확인형에서 "맞습니다" 또는 개방형에서 명확/충분**: 다음 프레임으로 이동
   - **확인형에서 "수정 필요"**: Other 입력 내용을 반영한 뒤 다음 프레임으로 이동 (필요시 파고들기)
   - **모호하거나 불완전**: 파고들기 질문을 생성하여 현재 위치에 이어서 진행
   - **"모르겠음" 선택**: ❓로 기록하고 다음 프레임으로 이동

3. 모든 프레임이 해소되면 C-4로 진행한다.
