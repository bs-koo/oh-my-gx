...
**저장 후 검증**: `python3 scripts/plan-lint.py .dev/plan.md`를 실행해 통과를 확인한다 (플러그인 저장소 경로 기준이므로 소비 프로젝트에서는 스크립트 경로를 맞춰 실행하거나, 스크립트가 없으면 건너뛴다). ID 중복·미존재 의존·순환은 여기서만 잡힌다 — 파이프라인의 phase-setup은 행을 찾을 뿐 그래프를 검사하지 않으므로, 잘못된 계획이 그대로 의존 확인의 근거가 된다. 실패하면 사용자에게 사유를 보고하고 수정 후 다시 저장한다. 재계획(C-5-6) 시에도 같은 검증을 수행한다.
