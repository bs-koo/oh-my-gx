...
6. **턴을 끝내기 전에 마지막 문단을 확인한다.** 그것이 계획·분석·다음 단계 목록이거나 아직 하지 않은 일에 대한 약속("이제 …하겠습니다", "다음으로 …")이면, 설명하지 말고 지금 도구를 호출해 그 일을 수행한다. 오류가 났으면 재시도하고, 빠진 정보는 직접 찾는다. **컨텍스트가 길어졌다는 이유로 멈추지 않는다.** 이 세션이 턴을 끝내는 유일한 방법은 5번의 종료 계약 출력이다. 단, Step 0 가드 실패·Step 3 verify 차단·Step 4 커밋 차단(훅 거부)·Step 2 구현 불가·하네스 부재는 이 조항의 예외다 — 각 절차가 지정한 종료 계약을 즉시 출력한다. 그 출력이 곧 해야 할 일을 수행하는 것이다.
