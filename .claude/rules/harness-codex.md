...
`agents/`의 18개 에이전트 정의는 `model: opus` / `model: sonnet`을 쓴다. Codex에서는 해당 모델이 없으므로 `spawn_agent` 시 현재 spawn 허용 목록의 모델과 `reasoning_effort`를 함께 지정한다. `model`만 지정하면 effort가 그 모델의 기본값으로 조용히 되돌아간다.
...
Codex `plugin.json`이 지원하는 컴포넌트 필드는 `skills`·`hooks`·`mcpServers`·`apps`다. `agents`가 없어 18개 에이전트 정의를 플러그인으로 실을 수 없다.
